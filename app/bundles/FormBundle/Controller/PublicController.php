...
class PublicController extends CommonFormController
{
    ...
    public function submitAction()
    {
        ...
        $return        = (isset($post['return'])) ? $post['return'] : false; // where return?


        // ---------------------------------------
        // new get of custom form in indivy.
        // added in indivy.
        // ---------------------------------------

        $newPost = $this->request->request->all();
        if(count($newPost)> 0) $post = $newPost;

        if($this->request->request->get('form_id'))
        {
            $post["form_id"] = $this->request->request->get('form_id');
        }
        ...
        if (!isset($post['formId']) && !isset($post["form_id"])) {
            $error = $translator->trans('mautic.form.submit.error.unavailable', [], 'flashes');
        } else {
            $formModel = $this->getModel('form.form');
            if(isset($post["form_id"]))
            {
                $Aliasentity = $formModel->getRepository()->findBySearch_Alias($post["form_id"]);
                $form = $Aliasentity[0];
            }
            else
            {
            ...
            if ($form === null) {
                $error = $translator->trans('mautic.form.submit.error.unavailable', [], 'flashes'); // entity not found
            } else {
                ...
                if ($status == 'pending') {
                    ...
                    foreach($post as $key => $value){
                        $this->factory->getLogger()->log('warning', $key . " : " . $value);
                    }
