 ...
 */
class BusinessGroupController extends FormController
{
    /**
     * Generate's default businessgroup list view.
     *
     * @param int $page
     *
    ...
    public function indexAction($page = 1)
    {
        ...
        if (!$this->get('mautic.security')->isGranted('user:businessgroup:view')) {
        ...
        //set limits
        $limit = $this->get('session')->get('mautic.businessgroup.limit', $this->coreParametersHelper->getParameter('default_pagelimit'));
        ...
        if ($count && $count < ($start + 1)) 
        {
            ...
            $this->get('session')->set('mautic.businessgroup.page', $lastPage);
            $returnUrl = $this->generateUrl('mautic_businessgroup_index', ['page' => $lastPage]);
        ...
        foreach ($items as $businessgroup) {
            $businessGroupIds[] = $businessgroup->getId();
        }

        $userCounts = (!empty($businessGroupIds)) ? $model->getRepository()->getUserCount($businessGroupIds) : [];

        //set what page currently on so that we can return here after form submission/cancellation
        $this->get('session')->set('mautic.businessgroup.page', $page);

        //set some permissions
        $permissions = [
            'create' => $this->get('mautic.security')->isGranted('user:businessgroup:create'),
            'edit'   => $this->get('mautic.security')->isGranted('user:businessgroup:edit'),
            'delete' => $this->get('mautic.security')->isGranted('user:businessgroup:delete'),
        ];
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => 'MauticUserBundle:BusinessGroup:list.html.php',
            'passthroughVars' => [
                'route'         => $this->generateUrl('mautic_businessgroup_index', ['page' => $page]),
                'mauticContent' => 'businessgroup',
            ],
        ]);
    }
    ...
    {
        //if (!$this->get('mautic.security')->isGranted('user:businessgroup:create')) {
        //    return $this->accessDenied();
        //}

        //retrieve the entity
        $entity = new Entity\BusinessGroup();
        $model  = $this->getModel('user.businessgroup');

        //set the return URL for post actions
        $returnUrl = $this->generateUrl('mautic_businessgroup_index');

        //set the page we came from
        $page   = $this->get('session')->get('mautic.businessgroup.page', 1);
        $action = $this->generateUrl('mautic_businessgroup_action', ['objectAction' => 'new']);
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->addFlash('mautic.core.notice.created', [
                        '%name%'      => $entity->getName(),
                        '%menu_link%' => 'mautic_businessgroup_index',
                        '%url%'       => $this->generateUrl('mautic_businessgroup_action', [
                            'objectAction' => 'edit',
    ...
    /**
     * Generate's businessgroup edit form and processes post data.
     *
     * @param int  $objectId
    ...
    {
        if (!$this->get('mautic.security')->isGranted('user:businessgroup:edit')) {
            return $this->accessDenied();
        }
        ...
        //set the return URL
        $returnUrl = $this->generateUrl('mautic_businessgroup_index', ['page' => $page]);
        ...
        if ($entity === null) {
            return $this->postActionRedirect(
                array_merge($postActionVars, [
                    'flashes' => [
                        [
                            'type'    => 'error',
                            'msg'     => 'mautic.user.businessgroup.error.notfound',
                            'msgVars' => ['%id' => $objectId],
                        ],
                    ],
                ])
            ...
            return $this->isLocked($postActionVars, $entity, 'user.businessgroup');
        }

        $permissionsConfig = $this->getPermissionsConfig($entity);
        $action            = $this->generateUrl('mautic_businessgroup_action', ['objectAction' => 'edit', 'objectId' => $objectId]);
        $form              = $model->createForm($entity, $this->get('form.factory'), $action, ['permissionsConfig' => $permissionsConfig['config']]);
        ...
        if (!$ignorePost && $this->request->getMethod() == 'POST') {
            ...
            if (!$cancelled = $this->isFormCancelled($form)) {
                if ($valid = $this->isFormValid($form)) {
                    //set the permissions
                    $permissions = $this->request->request->get('[permissions]', null, true);
                    $model->setRolePermissions($entity, $permissions);
     ...
     * @return array
     */
    private function getPermissionsConfig(Entity\BusinessGroup $role)
    {
