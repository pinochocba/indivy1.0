 ...
 * RoleRepository.
 */
class BusinessGroupRepository extends CommonRepository
{
    /**
     * Get a list of roles.
     *
    ...
    public function getRoleList($search = '', $limit = 10, $start = 0)
    {
        ...
        $q->select('partial b.{id, name}')
            ->from('MauticUserBundle:BusinessGroup', 'b');
    ...
    public function getUserCount($roleIds)
    {
        ...
        $q->where(
            $q->expr()->in('u.businessgroup_id', $roleIds)
        )
            ->groupBy('u.businessgroup_id');
